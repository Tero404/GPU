...
class Planet:
    ...
    def draw(self, screen):
        if len(self.orbit) > 1:
            pygame.draw.lines(surface=screen, color=self.color, closed=False, points=self.orbit, width=2)
...
def main():
    ...
    planets_df = pd.DataFrame([
        ['sun',     0.000 * AU, 0, 3, YELLOW,     1.988922 * 10**30, 0.0,  0.000 * 1000, True],
        ['mercury', 0.387 * AU, 0, 3, DARK_GREY,  3.300000 * 10**23, 0.0, -47.400 * 1000, False],
        ['venus',   0.723 * AU, 0, 3, WHITE,      4.8685   * 10**24, 0.0, -35.020 * 1000, False],
        ['earth',   -1.000 * AU, 0, 3, BLUE,       5.974200 * 10**24, 0.0,  29.783 * 1000, False],
        ['mars',    -1.524 * AU, 0, 3, RED,        6.390000 * 10**23, 0.0,  24.077 * 1000, False],
        ['jupiter', -5.203 * AU, 0, 3, ORANGE,     1.8982   * 10**27, 0.0,  13.070 * 1000, False],
    ...
    planets = torch.from_numpy(planets_df[['x','y','mass', 'x_velocity', 'y_velocity']].to_numpy()).to(device)
    ...
    for index, row in planets_df.iterrows():
        planet_obj = Planet(row['body'], row['x'], row['y'], row['size'], row['color'], row['is_sun'])
    ...
    while run:
        ...
        coords, velocities, distance_to_sun = update_positions(coords, masses, velocities, G, TIMESTEP)
